...
import type { WeaponService } from "../application/WeaponService";
...
export class WeaponController {
  ...
  async create(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        !weapon_element ||
        !weapon_type ||
        weapon_base_damage === undefined ||
        isNaN(weapon_base_damage)
      ) {
        handleResponse(res, {
          error:
            "All fields (weapon_name, weapon_element, weapon_type, weapon_base_damage) are required and must be valid.",
          statusCode: 400,
        });
        return;
      }
  ...
  async updateInfo(req: Request, res: Response): Promise<void> {
    logger.info(
      `Updating weapon info with payload: ${JSON.stringify(req.body)}`,
    );
    try {
      const weapon_id = req.params.id;
      const { weapon_name, weapon_element, weapon_type, weapon_base_damage } =
        req.body;
      ...
      if (
        ...
        handleResponse(res, {
          error:
            "All fields (weapon_id, weapon_name, weapon_element, weapon_type, weapon_base_damage) are required and must be valid.",
          statusCode: 400,
        });
        return;
      }
  ...
  async updateImage(req: Request, res: Response): Promise<void> {
    logger.info(
      `Updating weapon image with payload: ${JSON.stringify(req.body)}`,
    );
    try {
      const weapon_id = req.params.id;
      const weapon_image = req.file?.buffer;
      ...
      if (!weapon_id || !weapon_image) {
        handleResponse(res, {
          error:
            "All fields (weapon_id, weapon_image) are required and must be valid.",
          statusCode: 400,
        });
        return;
      }
