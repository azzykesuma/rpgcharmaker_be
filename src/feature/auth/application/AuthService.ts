...
import type { TCreateUserPayload, UserRepository } from "../domain/User";
import { SessionRepository } from "../infrastructure/repositories/SessionRepository";
import { RefreshTokenRepository } from "../infrastructure/repositories/RefreshTokenRepository";

import logger from "../../../shared/utils/logger";
import { compareHashPassword } from "../../../shared/utils/compareHashPassword";
import { JWT_SECRET_KEY, REFRESH_SECRET_KEY } from "../../../shared/constant";
...
export class AuthService {
  ...
  async login(
    ...
    if (!user) {
      logger.warn(
        `[AuthService] Login failed: User not found for username: ${username}`,
      );
      throw new Error("User not found");
    }
    logger.info(
      `[AuthService] User found for username: ${username}, verifying password...`,
    );

    const isPasswordValid = await compareHashPassword(
      password,
      user.player_password,
    );
    if (!isPasswordValid) {
      logger.warn(
    ...
    logger.info(
      `Generating access and refresh tokens for user id: ${user.user_id}`,
    );
  ...
  async refreshToken(token: string) {
    ...
    const decoded = jwt.verify(token, REFRESH_SECRET_KEY) as {
      id: string;
      username: string;
    };
