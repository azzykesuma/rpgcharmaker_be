...
class AuthController {
    ...
    async login(req: Request, res: Response) {
        ...
        try {
            const { accessToken, refreshToken } = await this.authService.login(username, password, userAgent);
            handleResponse(res, {
    ...
    async register(req: Request, res: Response) {
        const { username, password } = req.body;
        logger.info(`[AuthController] Register request received for username: ${username}`);
        try {
            await this.authService.register({player_name: username, player_password: password});
            handleResponse(res, {
    ...
    async logout(req: Request, res: Response) {
        ...
        logger.info(`[AuthController] Logout request received for refresh token: ${refreshToken}`);
        try {
    ...
    async refreshToken(req: Request, res: Response) {
        const { refreshToken } = req.body;
        logger.info(`[AuthController] Refresh token request received for refresh token: ${refreshToken}`);
        try {
