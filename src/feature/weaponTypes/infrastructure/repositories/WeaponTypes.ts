import type {
  ICreateWeaponTypes,
  IWeaponTypesRepository,
} from "../../domain/WeaponTypes";
import type { Pool } from "pg";
import type { IWeaponTypes } from "../../domain/WeaponTypes";
...
export class WeaponTypesRepository implements IWeaponTypesRepository {
  ...
  async findAll(): Promise<IWeaponTypes[]> {
    ...
    const weaponTypes = await this.pool.query(
      "SELECT * FROM weapon_master_type",
    );
  ...
  async create(weaponTypes: ICreateWeaponTypes): Promise<IWeaponTypes> {
    const result = await this.pool.query(
      "INSERT INTO weapon_master_type (weapon_type_name) VALUES ($1) RETURNING *",
      [weaponTypes.weapon_type_name],
    );
  ...
  async findByName(weaponName: string): Promise<IWeaponTypes | null> {
    const result = await this.pool.query(
      "SELECT * FROM weapon_master_type WHERE weapon_type_name = $1",
      [weaponName],
    );
  ...
  async delete(weaponId: number): Promise<void> {
    const result = await this.pool.query(
      "DELETE FROM weapon_master_type WHERE weapon_type_id = $1 RETURNING *",
      [weaponId],
    );
