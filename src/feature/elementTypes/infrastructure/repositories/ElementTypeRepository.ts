...
export class ElementTypeRepository implements IElementTypesRepository {
    ...
    async create(elementTypes: ICreateElementTypes): Promise<IElementTypes> {
        const result = await this.pool.query("INSERT INTO element_types (element_types_name) VALUES ($1) RETURNING *", [elementTypes.element_types_name]);
        return result.rows[0];
    }

    async findByName(elementName: string): Promise<IElementTypes | null> {
        const result = await this.pool.query("SELECT * FROM element_types WHERE element_types_name = $1", [elementName]);
        return result.rows[0] || null;
    }

    async delete(elementId: number): Promise<void> {
        const result = await this.pool.query("DELETE FROM element_types WHERE element_types_id = $1 RETURNING *", [elementId]);
