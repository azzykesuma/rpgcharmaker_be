...
export class ResistanceController {
  ...
  async create(req: Request, res: Response): Promise<void> {
    logger.info(
      `ResistanceController: Creating resistance with payload: ${JSON.stringify(
        req.body,
      )}`,
    );
    try {
      ...
      if (!resistance_type || !resistance_damage_reduction) {
        handleResponse(res, {
          error:
            "All fields (resistance_type, resistance_damage_reduction) are required and must be valid.",
          statusCode: 400,
        });
        return;
      }
  ...
  async updateResistance(req: Request, res: Response): Promise<void> {
    logger.info(
      `ResistanceController: Updating resistance with payload: ${JSON.stringify(
        req.body,
      )}`,
    );
    try {
      ...
      if (!resistance_id || !resistance_damage_reduction) {
        handleResponse(res, {
          error:
            "All fields (resistance_id, resistance_damage_reduction) are required and must be valid.",
          statusCode: 400,
        });
        return;
      }
  ...
  async deleteResistance(req: Request, res: Response): Promise<void> {
    logger.info(
      `ResistanceController: Deleting resistance with id: ${req.params.id}`,
    );
    try {
      const { id } = req.params;
      const resistance = await this.resistanceService.deleteResistance(
        Number(id),
      );
      handleResponse(res, {
        data: resistance,
