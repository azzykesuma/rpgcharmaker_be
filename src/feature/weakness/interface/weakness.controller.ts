...
import type { IWeaknessCreate, IWeaknessUpdate } from "../domain/Weakness";
import type { WeaknessService } from "../application/weaknessService";
...
export class WeaknessController {
  ...
  async create(req: Request, res: Response): Promise<void> {
    logger.info(
      `WeaknessController: Creating weakness with payload: ${JSON.stringify(
        req.body,
      )}`,
    );
    try {
      ...
      if (!weakness_type || !weakness_damage_mult) {
        handleResponse(res, {
          error:
            "All fields (weakness_type, weakness_damage_mult) are required and must be valid.",
          statusCode: 400,
        });
        return;
      }
  ...
  async updateWeakness(req: Request, res: Response): Promise<void> {
    logger.info(
      `WeaknessController: Updating weakness with payload: ${JSON.stringify(
        req.body,
      )}`,
    );
    try {
      ...
      if (!weakness_id || !weakness_damage_mult) {
        handleResponse(res, {
          error:
            "All fields (weakness_id, weakness_damage_mult) are required and must be valid.",
          statusCode: 400,
        });
        return;
      }
  ...
  async deleteWeakness(req: Request, res: Response): Promise<void> {
    logger.info(
      `WeaknessController: Deleting weakness with id: ${req.params.id}`,
    );
    try {
      const weakness_id = Number(req.params.id);
