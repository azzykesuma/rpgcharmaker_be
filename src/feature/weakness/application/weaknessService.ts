...
import type { IWeakness, IWeaknessCreate, IWeaknessRepository, IWeaknessUpdate } from "../domain/Weakness.ts";
...
export class WeaknessService {
    ...
    async create(weakness: IWeaknessCreate): Promise<IWeakness> {
        try {
            ...
            const existingWeakness = await this.weaknessRepository.findWeaknessByType(weakness.weakness_type);
            if (existingWeakness) {
                throw new HttpError("Weakness type already taken", 400);
            }
            return await this.weaknessRepository.create(weakness);
        } catch (error) {
            throw new HttpError(error instanceof Error ? error.message : "An error occurred", 500);
        }
    }
